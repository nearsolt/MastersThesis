...
namespace MastersThesis {
    class GnuPlot {
#warning создать конфиг и вынести в него путь конфига
        public static string PathToGnuplot = @"C:\Soft\gnuplot\bin";
        ...
        static bool waitForFile(string filename, int timeout = 10000) {
            ...
            while (file == null) {
                try { file = new System.IO.StreamReader(filename); } catch {
                    if (attempts-- > 0)
                        Thread.Sleep(100);
                    else
                        return false;
