...
namespace MastersThesis {
    internal partial class MeshStore {
        ...
        internal MeshStore(List<NodeStore> inputNodeList, List<EdgeStore> inputEdgeList, List<TriangleStore> inputTriangleList, TweenAnimation lastAnimation, int decimalPlaces, int meshRefinementCoeff,
                           ref List<Node> outputNodeList, ref List<Edge> outputEdgeList, ref List<Triangle> outputTriangleList, ref List<TweenAnimation> outputAnimationList) {

            _nodeList = inputNodeList.ToList();
            _edgeList = inputEdgeList.ToList();
            _triangleList = inputTriangleList.ToList();

            foreach (NodeStore node in inputNodeList) {
                _outputNodeList.Add(new Node(node.NodeID, node.XCoordinate, node.YCoordinate));
            }
            _outputAnimationList.Add(lastAnimation);

            MeshRefinement(decimalPlaces, meshRefinementCoeff, inputTriangleList);
